...
def build_cdm_job_payload(
    ...
    Args:
        ...
        data_source: 数据来源标识字符串，用于填充 lgdsj_data_source 字段。
                     默认为 None，使用 "<示例委办局>-i<示例业务场景>-i本市智慧养老系统" 作为默认值。
...
def main():
    ...
    命令行参数（位置参数）：
        ...
        sys.argv[11]: data_source - 数据来源标识（可选，默认"<示例委办局>-i<示例业务场景>-i本市智慧养老系统"）
    ...
    try:
        # 从数据源数据库中提取字段列表（不再从Hive DDL解析）
        column_list, actual_from_table_name = get_column_list_from_data_source(
            from_link_name=from_link_name,
            schema_name=schema_name,
            table_name=from_table_name,
        )
        LOGGER.info("[cdm_job] 查询得到的字段列表: %s", column_list[:100] + "..." if len(column_list) > 100 else column_list)
        LOGGER.info(
